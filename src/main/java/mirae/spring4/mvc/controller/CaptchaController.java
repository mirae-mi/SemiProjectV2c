...
public class CaptchaController {
    ...
    public String cap02ok(HttpServletRequest req){
        //클라이언트에서 보낸 captcha 문자열을 받아냄
        String gCaptRes = req.getParameter("g-recaptcha");
        System.out.println("g-recaptcha=" + gCaptRes);

        //captcha 문자열의 적합성 여부 확인을 위해 비밀키와 captcha 문자열을 검증하는 url 작성
        String secretKey = "6Lfd8fYdAAAAAMwpUNPYKxkOVaoiwJ0jMJ1FNqqx";//내 시크릿 키로 바꿔주기!
        String verifyURL = "https://www.google.com/recaptcha/api/siteverify";
        String params = "secret=" + secretKey + "&response=" + gCaptRes;

        try {
            String jsonData = "";   //결과값 저장용 변수 선언
            ...
            BufferedReader br = new BufferedReader(new InputStreamReader(is, "UTF-8"));//응답결과 받아오기
            ...
            JSONParser parser = new JSONParser();//두번째 걸로 import
